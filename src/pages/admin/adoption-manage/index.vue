<template>
	<div class="container mx-auto px-4 md:px-12 pt-6 flex flex-col items-center mt-8">
		<div class="w-full text-right mb-6">
			<admin-create-pet ref="petCreateRef" :createShow="false" @afterSubmit="beforeSubmit" :county-show="false" :shelterShow="false" />
		</div>
...
import { IAdoption, IPet } from '~/service/modules/pet/types';
...
const getPetList = async (val: any) => {
	const { content } = await getAdoptionByShelterId(val.currentPage, val.pageSize, '');
